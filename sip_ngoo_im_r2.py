...
def search_han_ji(un: str, tiau: str, siann: str):
    """ 檢索符合條件的漢字 """
    results = df_dict[(df_dict["韻"] == un) & (df_dict["調"].astype(str) == tiau) & (df_dict["聲"] == siann)]
...
class SipNgooIm:
    def __init__(self, root):
        ...
        self.combo_un_mu = ttk.Combobox(root, textvariable=self.un_var, font=font_style)
        ...
        self.combo_tiau = ttk.Combobox(root, textvariable=self.tiau_var, font=font_style)
    ...
    def select_un_mu_by_enter(self, event):
        ...
        typed_text = self.un_var.get()
        if typed_text in un_mu_dict:
            self.combo_un_mu.set(un_mu_dict[typed_text])  # 自動選擇對應的韻母
        elif any(v.startswith(typed_text) for v in un_mu_list):
            closest_match = next(v for v in un_mu_list if v.startswith(typed_text))
            self.combo_un_mu.set(closest_match)  # 選擇最相近的韻母

    def update_tiau_display(self, event):
        """根據輸入的數字更新聲調顯示"""
        typed_text = self.tiau_var.get()
        if typed_text in tiau_dict:
            self.combo_tiau.set(tiau_dict[typed_text])  # 顯示對應的中文調號
        elif any(k.startswith(typed_text) for k in tiau_dict):
            closest_match = next(tiau_dict[k] for k in tiau_dict if k.startswith(typed_text))
            self.combo_tiau.set(closest_match)  # 顯示最相近的調號
    ...
    def search_han_ji(self):
        # 獲取使用者輸入的原始值
        un_input = self.un_var.get().strip()  # 韻母輸入值
        tiau_input = self.tiau_var.get().strip()    # 聲調輸入值
        siann_input = self.siann_var.get().strip()  # 聲母輸入值

        # 將聲調的中文顯示值轉換回阿拉伯數字
        tiau = next((k for k, v in tiau_dict.items() if v == tiau_input), tiau_input)

        # 將聲母的漢字顯示值轉換回羅馬拼音字母
        siann = next((k for k, v in siann_dict.items() if v == siann_input), siann_input)

        # 將韻母的漢字顯示值轉換回羅馬拼音字母
        un = next((k for k, v in un_mu_dict.items() if v == un_input), un_input)

        # 執行搜尋
        results = search_han_ji(un, tiau, siann)
